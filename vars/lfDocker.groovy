...
def call(body) {
    ...
    lfDockerLogin(settingsFile=config.mvnSettings)
    ...
    if (env.GIT_BRANCH == "main" || env.GIT_BRANCH == "master") {
        push(config.project, config.containerPushRegistry)
    }
}
...
def dockerBuild(dockerImageName) {
    ...
    // On Jenkins systems we need to reset file ownership before build
 ...
 *   Optional env variables:
 *     * DOCKER_BUILD_ARGS: Build-time arguments to pass to Docker.
 *     * DOCKER_CUSTOM_TAGS: Space-separated string of additional tags. Used by getDockerTags.
 *     * VERSION: If version is not being set through another means, it can be passed in.
 *
 ...
 */
def push(dockerImage, registry, latest = true, tags = null) {
    if (tags == null) {
